...
```Plain
当 sys_user 为空且开启管理员引导配置时，
第一个注册成功的用户自动成为 SYSTEM_ADMIN，用于后续系统用户管理。
```
...
已经属于某个工作空间、但没有任何 OWNER / ADMIN 身份时不允许创建。
...
已经是某个工作空间的 `MEMBER`、但没有任何 OWNER / ADMIN 身份的用户不能创建新工作空间，返回 `40301`。
